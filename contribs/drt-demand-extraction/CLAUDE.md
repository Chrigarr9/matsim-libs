...
- `reposTimeMeanOutgoing` — DONE. The sole surviving output of the handoff pass: mean repositioning travel time over the top-K kept successors (`maxSuccessors`, default 50), or `-1.0` when a ride has no feasible successor. Consumed by the `chained_timebin` fleet estimator.
  - **K = 50 is calibrated, not arbitrary** — see `../../../docs/paper1/methodology-review/2026-08-01-repos-time-k-calibration.md`. A 20x sweep (K ∈ {10, 50, 200}) over one identical candidate stream moves the mean by 1.2% and moves the fleet estimate by less than the MIP's own alternate-optima noise. Do not tune K hoping to move the fleet number.
  - **Known limitation:** `chained_timebin` reports ~80% MORE vehicles than the post-MIP path cover on the Lyon 10% sample (117 vs 65). The causes are structural, not K: Java's handoff filter (distance cap 0.25) is stricter than the path cover's, so 42% of realised handoffs have no estimate at all; the estimator cannot represent depot bridging (29% of realised edges); and it averages over candidate successors rather than the chosen one. Prefer `pc_fleet_size` where both are available.

...
- `successors` and `predecessors` columns — **both removed from the Java emit** (2026-07-31). `predecessors` was a redundant reverse map Python dropped on load; `successors` was overwritten unconditionally by `solver_input_builder.py` with edges recomputed over the MIP-selected ride set, so the static top-K edges were never read on the Paper 1 path. Materialising them cost ~48 GB of collections at the 100% pool.
  - **Consequence:** `fleet_method: routing` no longer works — `gurobi.py` and `highs.py` raise "Routing fleet sizing requires successors data". Use `chained_timebin` (which needs only `reposTimeMeanOutgoing`) or restore the column.
- `reposTimeMeanOutgoing` — DONE. The sole surviving output of the handoff pass: mean repositioning travel time over the top-K kept successors (`maxSuccessors`, default 50), or `-1.0` when a ride has no feasible successor. Consumed by the `chained_timebin` fleet estimator.
  - **K = 50 is calibrated, not arbitrary** — see `../../../docs/paper1/methodology-review/2026-08-01-repos-time-k-calibration.md`. A 20x sweep (K ∈ {10, 50, 200}) over one identical candidate stream moves the mean by 1.2%, because the top-K heap is rarely full (59.6% of rides have <10 feasible successors, 80.3% <50). Do not tune K hoping to move the fleet number.
  - **Who consumes this column:** BOTH Python paths, as of 2026-08-01. The direct gurobi/highs path reads it via `resolve_repos_time()`; the colgen path reads it via `DuckDBRidePool.build_repos_time()`, the SQL mirror of that function (the two are pinned equal per ride by `tests/integration/test_repos_time_parity.py`). Until 2026-08-01 colgen ignored it entirely and applied a flat `repos_offset = timebin_size` to every ride, so K could not affect a colgen fleet number at all -- that is no longer true, but it is why the K calibration's fleet half was inconclusive. On both paths the value is now only an UPPER bound: a nearer depot can cap it (`min(java_mean, depot_pad)`), which fires on ~0.3% of rides.
  - **Estimate quality (direct path).** Scored against the path cover's realised handoffs on Lyon 10%: MAE ~165 s, bias ~-55 s (under-estimates per edge), and **42% of realised handoffs have no estimate at all** because the filter used for that corpus (900 s + distance cap 0.25) is stricter than the path cover's (1800 s, no cap). Note the 900 s is NOT the Java default -- `predecessorsFilterTime` defaults to 1800.0 and every other run script passes 1800; only `scripts/run_rural_100pct_nomaxdeg_d8_export.sh` overrides it to 900. So re-measure this figure before quoting it as a property of the method. The asymmetry is a tractability compromise for the 100% run, not a modelling choice — the path cover can afford to be generous because it runs on a few hundred already-selected rides.
- Network export for empty vehicle distances — DONE (`connection_cache.csv`; populated as a side-effect of the handoff enumeration pass)

The path cover's edge-feasibility ladder requires the cache to cover handoff pairs `(last-destination of i, first-origin of j)` within the configured `predecessorsFilterTime` window (default 1800 s). The `connectionCacheExportMode` option chooses what to emit (validated set: `window`/`all`): `"window"` (default) writes only the rows within the `predecessorsFilterTime` handoff window — the memory-bounded mode used by the 100% path; `"all"` writes everything BAMAS routed (broadest). A former `"successors_only"` mode was removed with the column it read. A "handoffs" mode (write rows whose endpoints match any kept ride's last-destination × first-origin — the lookup domain of Python's `compute_dynamic_successors`) is implemented offline in `scripts/post_topk_prune.py --cache-mode handoffs`.

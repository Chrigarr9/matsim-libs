...
- `reposTimeMeanOutgoing` — DONE. The sole surviving output of the handoff pass: mean repositioning travel time over the top-K kept successors (`maxSuccessors`, default 50), or `-1.0` when a ride has no feasible successor. Consumed by the `chained_timebin` fleet estimator.
  ...
  - **Who actually consumes this column:** only Python's **direct gurobi/highs** path, via `resolve_repos_time()`. The **colgen** path ignores it entirely and applies a flat `repos_offset = timebin_size` to every ride, so K cannot affect a colgen fleet number at all. Any future K experiment must run on gurobi/highs to mean anything.
  - **Estimate quality (direct path).** Scored against the path cover's realised handoffs on Lyon 10%: MAE ~165 s, bias ~-55 s (under-estimates per edge), and **42% of realised handoffs have no estimate at all** because Java's filter (900 s + distance cap 0.25) is stricter than the path cover's (1800 s, no cap). That asymmetry is a tractability compromise for the 100% run, not a modelling choice — the path cover can afford to be generous because it runs on a few hundred already-selected rides.

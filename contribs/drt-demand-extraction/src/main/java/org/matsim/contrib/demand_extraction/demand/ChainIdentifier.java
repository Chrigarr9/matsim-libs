...
public class ChainIdentifier {
    ...
    private boolean subtourUsesPrivateVehicle(TripStructureUtils.Subtour subtour,
        ...
        double bestNonPrivateUtility = 0.0;
        boolean hasNonPrivateForAllTrips = true;
        ...
        for (int tripIdx : subtourTripIndices) {
            Map<String, ModeAttributes> tripModes = personModeAttributes.get(tripIdx);
            if (tripModes == null) {
                // No modes available at all - shouldn't happen
                hasNonPrivateForAllTrips = false;
                break;
            }
            ...
            if (bestTripUtility == Double.NEGATIVE_INFINITY) {
                ...
                // This means only private vehicle modes (car/bike) are available
                // In this case, private vehicle must be used for this subtour
                hasNonPrivateForAllTrips = false;
                break;
            }
        ...
        // If no non-private modes available for any trip, private vehicle must be used
        // (if private vehicle is feasible, otherwise the subtour is infeasible but we return true
        // to ensure trips are grouped together for constraint checking)
        if (!hasNonPrivateForAllTrips) {
            return true;
        }

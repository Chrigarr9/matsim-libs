...
class HyperPoolStubRideOrderingTest {
	...
	private static void addRow(StopRideLayer cols, int[] set, int rideIdx) {
		int d = set.length;
	...
	void permutationSortsS2SByDegreeThenFirstPickupStably() {
		StopRideLayer d2 = new StopRideLayer(2);
		...
		StopRideLayer d3 = new StopRideLayer(3);
		addRow(d3, new int[]{2, 4, 6}, 88); // row 0: degree 3, firstPickup 2

		List<StopRideLayer> s2sLayers = List.of(d2, d3);
		int[][] perm = HyperPoolRideStore.computeSortPermutation(
	...
	void permutationIndependentOfPreFinalRideIndex() {
		List<StopRideLayer> a = buildTwoLayers(new int[]{77, 11, 33, 88});
		List<StopRideLayer> b = buildTwoLayers(new int[]{999, 111, 222, 444});

		int[][] permA = HyperPoolRideStore.computeSortPermutation(new ArrayList<>(), new ArrayList<>(), a);
		int[][] permB = HyperPoolRideStore.computeSortPermutation(new ArrayList<>(), new ArrayList<>(), b);
	...
	private static List<StopRideLayer> buildTwoLayers(int[] rideIdx) {
		StopRideLayer d2 = new StopRideLayer(2);
